...
var check = second();
var threshold = 0;
var bodyPartTotal = [0,0,0,0,0];
var armLeft = loadImage("armLeft.png");
var armRight = loadImage("armRight.png");
var footLeft = loadImage("footLeft.png");
var footRight = loadImage("footRight.png");
var head = loadImage("head.png");
var body = loadImage("body.png");
    ...
    if (fbc_array != undefined) {
        ...
        for (var i = 0; i < bars; i++) {
            ...
            for (var i = 0; i < bars; i++) {
                ...
                bodyPartTotal[Math.floor(i/(bars/bodyPartTotal.length))] += ((fbc_array[i]-threshold)/(255-threshold))
        ...
        pushMatrix();
        translate(1330,(350));
        rotate(-PI/2-((bodyPartTotal[3])*PI/1.5)+PI/3);
        image(armRight,0,120,57,315);
        popMatrix();
        pushMatrix();
        translate(1170,(350));
        rotate(-(-PI/2-((bodyPartTotal[2])*PI/1.5)+PI/3))
        image(armLeft,0,120,57,315);
        popMatrix();
        pushMatrix();
        translate(1170,(500));
        rotate(-(-PI/2-((bodyPartTotal[0])*PI/5)+PI/2))
        image(footLeft,0,150,105,271);
        popMatrix();
        pushMatrix();
        translate(1330,(500));
        rotate((-PI/2-((bodyPartTotal[1])*PI/5)+PI/2))
        image(footRight,0,150,105,271);
        popMatrix();
        pushMatrix();
        fill(255);
        stroke(255);
        translate(1250,300);
        image(body,0,110,217,319);
        translate(0,bodyPartTotal[3]*70)
        image(head,30,-40,185*1.2,125*1.2);
        popMatrix();
        popMatrix();
        if (style[1] == true) {
            noFill();
